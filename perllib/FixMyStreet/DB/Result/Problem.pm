...
sub duration_string {
    my ( $problem, $c ) = @_;
    my $body;
    if ( $c->cobrand->can('show_link_to_council_cobrand') &&
         $c->cobrand->show_link_to_council_cobrand($c, $problem) ) {
        my $base_url = $c->cobrand->get_body_handler_for_problem($problem)->base_url;
        my $url = sprintf("%s%s", $base_url, $problem->url);
